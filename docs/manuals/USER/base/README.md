...
<td>2</td>
<td>ファイルエリア</td>
<td><p>ファイル名のリンクとライセンス情報、シークレットURL機能を有効化している場合、シークレットURLボタンを表示します。</p><p>シークレットURボタンは、管理者画面でシークレットURL機能を有効化した際に表示されます。詳細は　[9.8 非公開コンテンツをワンタイムアドレスを利用して共有したい nnn](#非公開コンテンツをワンタイムアドレスを利用して共有したい)をご確認ください</p></td>
</tr>
<tr class="odd">
<td>3</td>
<td>ファイル情報エリア</td>
...
<td>5</td>
<td>［制限公開］</td>
<td>クリックしている場合は「表 5-8［ファイル情報］画面の項目と説明」の項番13～15を表示します。詳細については、「表 5-8［ファイル情報］画面の項目と説明」を参照してください。</p>
...
**本機能は、公開コンテンツ、ならびにサイトライセンス機能を利用してサイトライセンスユーザーに対してアクセス可能になっているコンテンツに対して、<span style="color: red; ">新たにアクセス権限を付与し、シークレットURLを通じたアクセスに制限する機能ではありません<span>。**
 ...
 下記の条件を満たすことで、Information画面のファイルエリアに「Secret URL」ボタンが追加されるのは、した場合となります。
 
 1.アイテム投稿者・アイテムの代理投稿者・リポジトリ管理者・システム管理者がログインしている。
 
 2.管理画面の制限公開画面にて、シークレットURLの表示を有効化している。
 
 3.当該ファイルを「公開しない」と設定している。または、ファイルが「オープンアクセス日を指定する」と設定し、指定日が未来日である。
 
 ![](media/media/images01.png)
 
 図9s1 シークレットURL機能を有効化した場合のInformation画面ファイルエリア

シークレットURL機能を有効化した場合、以下の項目が追加されます。

表 9‑s1 シークレットURL機能有効化時に表示される項目と説明
...
図 9s2 シークレッㇳURLボタンをクリックし、シークレットURL作成欄を表示した状態

シークレットURL作成欄の各項目の内容は以下の通りです。

表 9‑s2 シークレットURL作成欄の項目と説明
