...
CRIS機関と自動連携を行う手順について説明します。

1. 連携したいCRIS機関名の左部にあるチェックボックスにチェックを入れます。

　CRIS機関名の下には、連携結果が表示されています。表示結果は以下の4種となっています。
...
連携対象の著者に関しては、アイテムに入力してある著者（Creater,Contributer）を対象にします。連携には事前に著者DBにparmalinkを登録しておく必要があります。
