...
※サブリポジトリ管理者は、自身の管理下にあるサブリポジトリに関連付けられた著者のみ操作可能
...
1 Author IDを参照する
  ...
  - 【Administration\>著者DB管理（Author Management）\>編集（Edit）】での「Author ID」タブに登録されているAuthor IDが表示される。
      ...
      - サブリポジトリ管理者の場合は管理対象外の著者の「統合元チェックボックス」、「統合先ラジオボタン」、「編集ボタン」を非活性で表示され、押下しても動作しない。
...
2 Author IDを追加する。
  ...
  - 【Administration\>著者DB管理（Author Management）\>編集（Edit）】での「Author ID」タブに\[著者追加（Add Author）\]ボタンを押すと、著者追加画面に移動する。
      ...
      - 入力項目は以下の通りである。
          ...
          - 「コミュニティ」（Community）プルダウン：著者を管理するコミュニティを選択する。

              - 「コミュニティ管理」画面で登録されているコミュニティから選択できる。

              - サブリポジトリ管理者の場合は、管理しているコミュニティのみ選択できる。

              - \[+を追加（Add Community）\]ボタンを押すことで、コミュニティの入力欄が追加される。

              - \[X」ボタンを押すことで、コミュニティの入力欄が削除される。  
                表示されている入力エリアが１つのみの場合、削除不可とする。

              - ほかのコミュニティの入力欄で選択済みのコミュニティは選択肢に表示されない。
      ...
      - [強制変更フラグ(force change flag)]チェックボックス
          - 著者DB更新時のメタデータ自動更新機能：  
...
3 Author IDを編集・削除する。
  ...
  - Author ID一覧に\[編集（Edit）\]ボタンを押すと、Author IDの編集画面に移動する。
      ...
      - サブリポジトリ管理者が編集時に選択済みの管理対象外のコミュニティは編集することができない（グレーアウトとする）。
  ...
  - 著者の削除について
      ...
      - サブリポジトリ管理者の場合は、自身の管理するコミュニティに紐づかない、または管理するコミュニティ以外のコミュニティにも紐づいている著者は削除することができず、以下のポップアップメッセージが表示される。

          - 日本語：「著者IDに紐づく、コミュニティ「XXX」の管理権限がありません。」

          - 英語：「You do not have permission for this Author ID’s communities: XXX」
